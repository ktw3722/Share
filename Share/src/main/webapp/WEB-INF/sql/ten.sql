...
values
('ktw3722','남','개인','김성빈',1,1,1,1,1);
--트랜잭션요망
update member
set certi_num=(select sum(phone_certi+addr_certi+sns_certi+mail_certi+passport_certi) from member where id='ktw3722')
where id='ktw3722'
--------------------------------------read--------------------------
select id,gender,mem_type,name,certi_num,phone_certi,addr_certi,sns_certi,mail_certi,passport_certi 
from member
where id='ktw3722'
...
update member
set
phone_certi='1',addr_certi='1',sns_certi='1',mail_certi='1',passport_certi='0'
where id='ktw3722'
--트랜잭션요망
update member
set certi_num=(select sum(phone_certi+addr_certi+sns_certi+mail_certi+passport_certi) from member where id='ktw3722')
...
--------------------------------------list--------------------------
SELECT id,gender,mem_type,name,certi_num,phone_certi,addr_certi,sns_certi,mail_certi,passport_certi, r
		from(
			 SELECT id,gender,mem_type,name,certi_num,phone_certi,addr_certi,sns_certi,mail_certi,passport_certi, rownum r  
				from(SELECT id,gender,mem_type,name,certi_num,phone_certi,addr_certi,sns_certi,mail_certi,passport_certi  
						FROM member
						where name like '%김%' 
						ORDER BY id
			)
...
--------------------------------------------------------------------------
CREATE TABLE carpool_booked (
       cp_bookedno          NUMBER(10) NOT NULL,
       appli_ID             VARCHAR2(12char) NOT NULL,
       writer_ID            VARCHAR2(12char) NOT NULL,
       kind                 VARCHAR2(5char) NOT NULL
                                   CHECK (kind IN ('타세요', '태워주세요')),
       type                 VARCHAR2(6char) NOT NULL
                                   CHECK (type IN ('정기카풀', '단기카풀', '여성전용카풀')),
       appli_seat           NUMBER(1) NOT NULL,
       carpoolno            NUMBER NOT NULL,
       PRIMARY KEY (cp_bookedno), 
       FOREIGN KEY (carpoolno)
                             REFERENCES carpool
);
